...
class ExerciseLogProvider extends ChangeNotifier {
  ...
  /// Returns the most recent log for each exercise type (across all loaded logs).
  Map<String, ExerciseLog> getLatestByExerciseType() {
    final latest = <String, ExerciseLog>{};
    for (final log in _logs) {
      final clave = log.exerciseType?.clave;
      if (clave == null) continue;
      if (!latest.containsKey(clave) ||
          log.fecha.isAfter(latest[clave]!.fecha)) {
        latest[clave] = log;
      }
    }
    return latest;
